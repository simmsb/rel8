   ...
   instance Rel8able $(TH.conT name) where
          ...
          gfromColumns $( TH.varP contextName ) x =
            case $( TH.varE contextName ) of
              SResult    -> case x of $columnsP -> $(constructorE (\x -> TH.VarE 'runIdentity `TH.AppE` x) (\_ x -> TH.VarE 'undefined `TH.AppE` x))
          ...
          gtoColumns $(TH.varP contextName) $( constructorP ) =
            case $( TH.varE contextName ) of
              ...
              SResult    -> $(columnsE (\x -> TH.ConE 'Identity `TH.AppE` x) (\_ x -> TH.VarE 'undefined `TH.AppE` x))

          gfromResult $columnsP = undefined
          ...
          gtoResult $constructorP = undefined
              -- TODO: get rid of type application. Use a signature that references the generic value instead
